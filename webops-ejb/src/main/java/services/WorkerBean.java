...
public class WorkerBean {
	...
	public void InterviewPropertiesAlgo() throws InterruptedException {
		...
		TypedQuery<Interview> query = em.createQuery(
				"SELECT a FROM Interview a where (a.date IS NULL) AND (SELECT DATEDIFF(NOW(),x.date) FROM OnlineTest x WHERE x.candidatTest = a.candidatInterview  and x.state=:sss)>3",
				Interview.class);
	...
	public void AutoRefuseOnlineTest() throws InterruptedException {
		...
		Query query = em.createQuery("UPDATE OnlineTest o SET o.state=:s WHERE DATEDIFF(NOW(),o.date) >3 and o.state=:xx");
		query.setParameter("s", StateTestOnline.InValid);
		query.setParameter("s", StateTestOnline.WaitForIt);
		query.executeUpdate();
