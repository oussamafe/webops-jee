...
public class ApplicationCandidateManagementImplementation implements ApplicationCandidateManagementRemote {
	...
	public Set<Application> ViewAllApplicationStillWait() {
		TypedQuery<Application> query = em.createQuery("SELECT a FROM Application a where a.answerDate IS NULL", Application.class);
	...
	public Set<Application> ViewAllApplicationRejected() {
		TypedQuery<Application> query = em.createQuery("SELECT a FROM Application a where a.result=0 and a.answerDate IS NOT NULL",
				Application.class);
		Set<Application> results = new HashSet<Application>();
		results.addAll(query.getResultList());
		return results;
	...
	public int AcceptApplication(ApplicationId id) {
		Query query = em.createQuery("update Application e set e.result=:r,e.answerDate = NOW() where e.id=:i");
