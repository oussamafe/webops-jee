...
public class AvailabilityImplementation implements AvailabilityRemote {
	...
	public void InitialiseCandidateAvailability(int candidateID) {
		...
		for (int x = 0; x < 10; x++) {
			...
			if (localDate.getDayOfWeek().compareTo(DayOfWeek.SUNDAY) == 0) {
				...
				e.getAvailabilityCandidate().add(ac1);
				e.getAvailabilityCandidate().add(ac2);
				e.getAvailabilityCandidate().add(ac3);
			} else {
	...
	public void InitialiseEmployeAvailability(int EmployeID) {
		...
		for (int x = 0; x < 10; x++) {
			...
			if (localDate.getDayOfWeek().compareTo(DayOfWeek.SUNDAY) == 0) {
				...
				e.getAvailabilityEmploye().add(ae1);
				e.getAvailabilityEmploye().add(ae2);
				e.getAvailabilityEmploye().add(ae3);
			} else {
				if (localDate.getDayOfWeek().compareTo(DayOfWeek.FRIDAY) == 0
						|| localDate.getDayOfWeek().compareTo(DayOfWeek.SATURDAY) == 0) {
					AvailabilityEmploye ae1 = new AvailabilityEmploye();
	...
	@Override
	public void SetNoAvailibleCandidate(int candidateID, Date dateav, int heurDebut) {
		Candidate avaCan = em.find(Candidate.class, candidateID);

		Query query = em.createQuery(
				"UPDATE AvailabilityCandidate o SET o.state=:s WHERE o.date=:d and o.candidate=:c and o.start_hour=:sh");
		query.setParameter("s", false);
		query.setParameter("d", dateav);
		query.setParameter("c", avaCan);
		query.setParameter("sh", heurDebut);
		query.executeUpdate();
	}

	@Override
	public void SetAvailibleCandidate(int candidateID, Date dateav, int heurDebut) {
		Candidate avaCan = em.find(Candidate.class, candidateID);
	...
	@Override
	public void SetNoAvailibleEmploye(int EmployeID, Date dateav, int heurDebut) {
		Employe avaEmp = em.find(Employe.class, EmployeID);
		...
		query1.setParameter("d", dateav);
		query1.setParameter("e", avaEmp);
		query1.setParameter("sh", heurDebut);
		query1.executeUpdate();

	}

	@Override
	public void SetAvailibleEmploye(int EmployeID, Date dateav, int heurDebut) {
		Employe avaEmp = em.find(Employe.class, EmployeID);

		Query query1 = em.createQuery(
				"UPDATE AvailabilityEmploye o SET o.state=:s WHERE o.date=:d and o.employe=:e and o.start_hour=:sh");
		query1.setParameter("s", true);
		query1.setParameter("d", dateav);
		query1.setParameter("e", avaEmp);
		query1.setParameter("sh", heurDebut);
		query1.executeUpdate();
